...
public class WheatTimer extends BukkitRunnable {

    public WheatTimer(Block block, Player p) {
    ...
    public void run() {
        CropState state = new CropStateImp();
        if (second == 0) {
            if (num == 50 || num == 40 || num == 30 || num == 20 || num == 10 || num == 0) {
        ...
        for (Entity entity : world.getEntities()) {
            if (entity instanceof ArmorStand) {
                ...
                if (!armorStand2.getLocation().equals(block2.getLocation())) {
                    ...
                    armorStand.setCustomName(num + "분" + "" + second + "초");
